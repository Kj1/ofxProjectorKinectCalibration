...
void testApp::setup() 
{
	...
	kinectCalibratedColorImage.allocate(640, 480);
	kinectLabelImageGray.allocate(640, 480);
    
    threshold = 125;
    
	//make the wrapper (to make calibration independant of the drivers...)
	RGBDCamCalibWrapper* kinectWrapper = new RGBDCamCalibWrapperOfxOpenNi();
...
void testApp::update() 
{
	...
	kinect.update();
	kinectCalibratedColorImage.setFromPixels(kinect.getImagePixels());
    
	//kinectLabelImageGray.setFromPixels(kinect.getDepthPixels(), 640, 480);
    ofPixels& imgPixels = kinectLabelImageGray.getPixelsRef();
    ofShortPixels& kinectPixels = kinect.getDepthRawPixels();
    unsigned char p;
    const short minDepth = 450;
    const short maxDepth = 2000;
    for (int i=0; i<640*480; i++) {
		short s = kinectPixels[i];
		char x = (s < minDepth || s > maxDepth) ? 255 : powf(s - minDepth, 0.7f);
		imgPixels[i] = 255 - x;
    }
    kinectLabelImageGray.flagImageChanged();
